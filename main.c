...
int maxValueFromHere(Artifact* allArts, int numArts, int here, int weightAllow) {
    ...
    if (here == numArts)
        return 0;
    if (weightAllow - allArts[here].weight < 0)
        return maxValueFromHere(allArts, numArts, here+1, weightAllow);
    
    int inclusive = allArts[here].value + maxValueFromHere(allArts, numArts, here+1, weightAllow-allArts[here].weight);
    int exclusive = maxValueFromHere(allArts, numArts, here+1, weightAllow);
...
int maxValueFromHereFast(Artifact* allArts, int numArts, int here, int weightAllow, int** memo) {
    ...
    if (weightAllow - allArts[here].weight < 0)
        return maxValueFromHereFast(allArts, numArts, here+1, weightAllow, memo);
    
    int inclusive = allArts[here].value + maxValueFromHereFast(allArts, numArts, here+1, weightAllow-allArts[here].weight, memo);
    int exclusive = maxValueFromHereFast(allArts, numArts, here+1, weightAllow, memo);
    
    memo[here][weightAllow] = (inclusive > exclusive ? inclusive : exclusive);
    
    return memo[here][weightAllow];  // Modify this
}
